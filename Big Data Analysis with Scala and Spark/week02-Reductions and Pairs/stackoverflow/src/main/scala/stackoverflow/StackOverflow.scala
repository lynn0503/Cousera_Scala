...
//    parentId is Option, not Int, should use get method
  ...
  @tailrec final def kmeans(means: Array[(Int, Int)], vectors: RDD[(Int, Int)], iter: Int = 1, debug: Boolean = false): Array[(Int, Int)] = {
    ...
    val clusteredAvg=paired.groupByKey().mapValues(averageVectors).collect().toMap
    val newMeans=means.indices.map(i=>clusteredAvg.getOrElse(i,means(i))).toArray
...
//      val lang_index:LangIndex=vs.groupBy(_._1).mapValues(_.size).maxBy(_._2)._1
//      val langLabel: String   = langs(lang_index/langSpread)
//      // most common language in the cluster
//      val clusterSize: Int    = vs.size
//      val langPercent: Double = vs.count(_._1==lang_index)/clusterSize*100.0
//      // percent of the questions in the most common language in the cluster
//      val sortedList = vs.map(_._2).toList.sorted
//      val middle=clusterSize/2
//      val medianScore: Int    = if (middle%2==1) sortedList(middle) else (sortedList(middle-1)+sortedList(middle+1))/2
