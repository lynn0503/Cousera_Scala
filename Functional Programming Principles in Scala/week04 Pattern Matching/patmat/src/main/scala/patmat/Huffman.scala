...
import patmat.Huffman.{Fork, Leaf, encode}
...
object Huffman {
  ...
  /**
    ...
    *   }
    */
  def count(char: Char, cnt: List[(Char, Int)]): List[(Char, Int)] = cnt match {
    case List() => List((char, 1))
    case y :: ys =>
      if (char == y._1) (char, y._2 + 1) :: ys else y :: count(char, ys)
  }
  ...
  def insort(xs: List[(Char, Int)]): List[(Char, Int)] = xs match {
    case List()  => List()
    case y :: ys => insert(y, insort(ys))
  }

  def makeOrderedLeafList(freqs: List[(Char, Int)]): List[Leaf] = {
    def makeAcc(orderedFreq: List[(Char, Int)], acc: List[Leaf]): List[Leaf] =
      orderedFreq match {
        case List()  => acc
        case o :: os => makeAcc(os, acc :+ Leaf(o._1, o._2))
      }
    makeAcc(insort(freqs), List())
  }
  ...
  def combine(trees: List[CodeTree]): List[CodeTree] = {
    if (trees.length < 3) trees
      ...
      insertCodeTree(makeCodeTree(trees.head, trees.tail.head), trees.drop(2))
  }
  ...
  def decode(tree: CodeTree, bits: List[Bit]): List[Char] = {
    def traverse(tr: CodeTree, bts: List[Bit]): List[Char] =
      tr match {
        case Leaf(c, _) if bts.isEmpty            => List(c)
        case Leaf(c, _)                           => c :: traverse(tree, bts)
        case Fork(left, _, _, _) if bts.head == 0 => traverse(left, bts.tail)
        case Fork(_, right, _, _)                 => traverse(right, bts.tail)
      }
    traverse(tree, bits)
  }
  ...
  def encode(tree: CodeTree)(text: List[Char]): List[Bit] = {
    ...
    def lookUp(tree: CodeTree)(char: Char): List[Bit] = tree match {
      case Leaf(_, _) => List()
      case Fork(left, _, _, _) if chars(left).contains(char) =>
        0 :: lookUp(left)(char)
  ...
  def mergeCodeTables(a: CodeTable, b: CodeTable): CodeTable = {
    def prepend(bi:Bit,code:Code):(Char,List[Bit])=(code._1,bi::code._2)
    a.map(x=>prepend(0,x)) ::: b.map(x=>prepend(1,x))
  }
